...
import type { Ranking } from '@fixtura/types';
...
export function RankingPage({
  ...
  subtitulo,
  metricaLabel,
}: RankingPageProps): React.ReactElement {
  const { data, isLoading } = useRanking(ligaSlug, tipo);
  ...
  return (
    <>
      <PublicHeader
        ligaSlug={ligaSlug}
        ligaNombre={data?.torneo.nombre ?? 'Cargando…'}
        active={tipo as 'goleadores' | 'asistencias' | 'mvp'}
      />
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        {isLoading && (
          <div className="font-serif italic text-ink-mute">Cargando ranking...</div>
        )}
